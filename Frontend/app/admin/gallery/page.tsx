...
export default function AdminGalleryPage() {
  ...
  async function handleCreate(e: React.FormEvent) {
    ...
    try {
      ...
      fd.append("sortOrder", String(parseInt(createForm.sortOrder, 10) || 0));
      fd.append("active", createForm.active ? "true" : "false");
      const created = await uploadGalleryImage(fd);
      setItems((prev) => [...prev, created].sort((a, b) => (a.sortOrder ?? 0) - (b.sortOrder ?? 0)));
      setCreateForm({ title: "", category: "Classic", description: "", active: true, sortOrder: "0" });
      setCreateFile(null);
      setShowCreate(false);
  ...
  async function handleUpdate(e: React.FormEvent) {
    ...
    try {
      ...
      if (editFile) {
        ...
        fd.append("sortOrder", String(parseInt(editForm.sortOrder, 10) || 0));
        fd.append("active", editForm.active ? "true" : "false");
        updated = await replaceGalleryImage(editingId, fd);
      } else {
        updated = await updateGalleryItem(editingId, {
  ...
  async function handleDelete(id: string | number) {
    if (!confirm("Delete this gallery item? The Cloudinary image will be removed if applicable.")) return;
    try {
  ...
  return (
    <div>
      ...
      <p className="mt-2 text-muted">
        Upload images from your computer; files are stored on Cloudinary. Only active items appear on the public gallery.
      </p>
        ...
        {!loading && !error && filtered.length > 0 && (
          <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-3">
            {filtered.map((item) => (
              ...
              >
                <div className="flex aspect-[4/5] items-center justify-center bg-cream/50">
                  {item.imageUrl ? (
      ...
      {showCreate && (
        <>
          ...
          <div className="fixed left-1/2 top-1/2 z-50 max-h-[90vh] w-full max-w-md -translate-x-1/2 -translate-y-1/2 overflow-y-auto rounded-2xl border border-borderSoft/80 bg-white p-6 shadow-card">
            ...
            <form onSubmit={handleCreate} className="mt-4 space-y-3">
              <div>
                ...
                <input
                  ...
                  className="w-full text-sm text-charcoal file:mr-3 file:rounded-lg file:border-0 file:bg-gold/20 file:px-3 file:py-2 file:text-sm file:font-medium file:text-charcoal"
                />
              </div>
              <input
              ...
              <div className="flex gap-2 pt-2">
                <button
                  ...
                  className="flex-1 rounded-full border border-borderSoft/80 py-2.5 text-sm font-medium text-charcoal"
                >
                  Cancel
                </button>
                <button
                  type="submit"
                  disabled={saving}
                  className="flex-1 rounded-full bg-gold py-2.5 text-sm font-medium text-cream shadow-soft disabled:opacity-60"
                >
        ...
        <>
          ...
          <div className="fixed left-1/2 top-1/2 z-50 max-h-[90vh] w-full max-w-md -translate-x-1/2 -translate-y-1/2 overflow-y-auto rounded-2xl border border-borderSoft/80 bg-white p-6 shadow-card">
            ...
            <form onSubmit={handleUpdate} className="mt-4 space-y-3">
              <div className="overflow-hidden rounded-xl border border-borderSoft/40 bg-cream/30">
                <img src={editForm.imageUrl} alt="" className="aspect-[4/3] w-full object-cover" />
              </div>
              <div>
                <label className="mb-1 block text-xs font-medium text-muted">Replace image (optional)</label>
                <input
                  type="file"
                  accept="image/jpeg,image/png,image/gif,image/webp"
